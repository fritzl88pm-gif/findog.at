...
function renderRichBlock(block: RichBlock, index: number): ReactNode {
  ...
  if (block.type === "paragraph") {
    return (
      <p key={`paragraph-${index}`}>
        {renderRichInline(block.children, `paragraph-${index}`)}
      </p>
    );
  }
  ...
  if (block.type === "table") {
    return (
      <div className="fred-answer-table-scroll" key={`table-${index}`}>
        <table>
          <thead>
            <tr>
  ...
  return (
    <blockquote className="fred-answer-callout" key={`blockquote-${index}`}>
      {renderRichInline(block.children, `blockquote-${index}`)}
    </blockquote>
  );
}

export function RichAnswer({ content }: { content: string }) {
  const blocks = parseRichAnswer(content);

  if (blocks.length === 0) {
    return <p className="fred-message-body" />;
  }

  return (
    <div className="fred-answer-content">
      {blocks.map(renderRichBlock)}
    </div>
  );
}
