...
const characterSpriteLayouts: Record<FredRunCharacterId, Record<SpriteKey, CharacterSpriteLayout>> = {
  ...
  superfrida: {
    walk: { source: "/fredrun/superfrida/walk.webp?v=provided-sheet-1", columns: 8, frameCount: 64, fps: 16 },
    jump: { source: "/fredrun/superfrida/jump.webp?v=provided-sheet-1", columns: 8, frameCount: 64, fps: 16 },
...
function renderFredRun(
  ...
  state.platforms?.forEach((platform) => drawPlatform(context, platform, state.elapsed, reducedMotion, Math.min(1, Math.max(0, (fredRunContinuousScoreForDistance(state.distance) - 750) / 500))));
  ...
  state.obstacles.forEach((obstacle) => {
    if (effectiveWorldId === "finanzamt-night" && isFredRunCabinet(obstacle)) {
      drawFilingCabinet(context, obstacle, state.elapsed, reducedMotion);
    } else if (images) {
      drawObstacle(context, obstacle, images.obstacles[obstacle.kind], state.elapsed, reducedMotion);
    }
  });
