...
describe("Admin OmniRoute Stats UI", () => {
  ...
  it("authorizes only with the Supabase access token and supports range and refresh controls", () => {
    ...
    expect(componentSource).toContain('|| !("routes" in payload)');
  });

  it("renders loading, error, stale and empty states with required German labels and no obsolete hardcoded sections", () => {
    for (const label of [
      "Codex OAuth Quota",
      "Gemini Flash Pool",
      "Verbleibend",
    ...
    expect(componentSource).toContain("Keine konfigurierten Routen gefunden.");

    // Absence of obsolete hardcoded sections and fixed labels
    expect(componentSource).not.toContain("Fred V4 Stack");
    expect(componentSource).not.toContain("Primäres Ziel (Luna Max via Codex OAuth)");
    expect(componentSource).not.toContain("Fallback-Ziel (Gemini 3.7 Flash High)");
    expect(componentSource).not.toContain("Aktive Route");
    expect(componentSource).not.toContain("call-logs");
  });
