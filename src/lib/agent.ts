...
function withoutLegacyPdfOffer(result: AgentRunResult): AgentRunResult {
  const publicResult = { ...result };
  delete publicResult.pdfOffer;
  return publicResult;
}
...
export async function runAgent(options: RunAgentOptions): Promise<AgentRunResult> {
  ...
  const creationMessages = (repairReasons: readonly string[] = []): DeepSeekMessage[] => [
    { role: "system", content: options.systemPrompt },
    {
      ...
      content: [
        ...
        "Der PDF-Inhalt ist ein eigenständiges Dokument und nicht bloß die sichtbare Chatantwort oder eine Download-Zusage.",
        "Nutze ausschließlich den vorliegenden Gesprächskontext und die bereits recherchierte aktuelle Agentenantwort; ergänze keine neuen Rechtsaussagen oder Fundstellen.",
        ...
        "",
        "Bereits recherchierte aktuelle Agentenantwort:",
        result.answer,
      ].filter(Boolean).join("\n"),
    },
  ];
  ...
  let drafts: PdfArtifactDraft[] = [];
  for (let attempt = 0; attempt < 2 && drafts.length === 0; attempt += 1) {
    const creationResult = await chatCompletion({
      runtime: options.runtime,
      deadline: options.deadline,
      messages: creationMessages(repairReasons),
  ...
  await appendAgentStep(
    ...
    {
      type: "tool_result",
      title: drafts.length === 1 ? "PDF-Dokument vorbereitet" : "PDF-Dokumente vorbereitet",
      content: drafts.map((draft) => draft.title).join("\n"),
      toolName: CREATE_PDF_DOCUMENT_TOOL_NAME,
      success: true,
    },
    options.onStep,
