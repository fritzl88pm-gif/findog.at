...
describe("Fred conversation deletion RPC migrations", () => {
  ...
  it("clears all ledger links and redacts all owned receipts atomically before deleting conversations", () => {
    ...
    const ledgerUpdate = latestRpc.definition.match(
      /update public\.fred_request_ledger as receipt\s+set[\s\S]*?where receipt\.client_id = p_client_id\s+and receipt\.id = any\(request_ids\);/i,
    )?.[0] ?? "";
    ...
    expect(ledgerUpdate).toMatch(/request_content = null/i);
    expect(ledgerUpdate).toMatch(/request_content_sha256 = null/i);
    expect(ledgerUpdate).toMatch(/conversation_id = null/i);
    expect(ledgerUpdate).toMatch(/user_message_id = null/i);
    expect(ledgerUpdate).toMatch(/assistant_message_id = null/i);
    expect(ledgerUpdate).toMatch(/content_deleted_at = coalesce\(receipt\.content_deleted_at, deletion_time\)/i);
    expect(ledgerUpdate).toMatch(/'user_conversation_delete'/i);
  ...
  it("preserves security, owner scope, account-wide cleanup, and grants", () => {
    ...
    expect(latestRpc.definition).toMatch(/cardinality\(p_conversation_ids\) < 1/i);
    expect(latestRpc.definition).not.toMatch(/cardinality\(p_conversation_ids\) > 100/i);
