-- Fred research display mode setting and message execution trace persistence.
-- Extends fred_user_preferences with research_display_mode ('simple' | 'advanced')
-- and fred_messages with execution_trace jsonb.

-- 1. Add research_display_mode to fred_user_preferences --------------------

alter table public.fred_user_preferences
  add column if not exists research_display_mode text not null default 'simple';

do $$
begin
  if not exists (
    select 1 from information_schema.table_constraints
    where constraint_name = 'fup_research_display_mode_check'
      and table_schema = 'public'
      and table_name = 'fred_user_preferences'
  ) then
    alter table public.fred_user_preferences
      add constraint fup_research_display_mode_check
      check (research_display_mode in ('simple', 'advanced'));
  end if;
end $$;

-- 2. Add execution_trace to fred_messages ----------------------------------

alter table public.fred_messages
  add column if not exists execution_trace jsonb not null default '[]'::jsonb;

-- 3. Update record_fred_native_event to persist execution_trace ------------
...
declare
  ...
  existing_execution_trace jsonb;
...
begin
  ...
  execution_trace_value := coalesce(payload -> 'execution_trace', '[]'::jsonb);
  ...
  if jsonb_typeof(research_trace_value) is distinct from 'array'
    ...
    or jsonb_typeof(execution_trace_value) is distinct from 'array'
    or jsonb_array_length(execution_trace_value) > 200
  ...
  if event_type_value = 'message_received' then
    ...
    if display_content_value is not null
      ...
      or execution_trace_value <> '[]'::jsonb
  ...
  if metadata_already_recorded
    and (
      ...
      or existing_execution_trace is distinct from execution_trace_value
  ...
  set attachments = attachments_value,
      ...
      execution_trace = execution_trace_value,
