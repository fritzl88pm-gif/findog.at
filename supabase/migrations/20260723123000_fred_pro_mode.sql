-- Persists the Fred Pro mode flag as bounded request metadata.
-- Preserves every existing role/content/metadata constraint and
-- extends the service-only native RPC without weakening its security model.
alter table public.fred_messages
  add column pro_mode_enabled boolean not null default false;

alter table public.fred_messages
  add constraint fred_messages_pro_mode_role
    check (
      role = 'user'
      or pro_mode_enabled = false
    );
