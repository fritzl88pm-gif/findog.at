...
const SOURCE_PATH = process.argv[2] ?? "C:/Users/conta/Downloads/luki-kolumbian_lauf.png";
const EXPECTED_SOURCE_HASH = "3D715AF687931CC29FAD5EC1B2FD5E08D0617ADA253B9D03F027128DFDF44F62";
const OUTPUT_DIRECTORY = path.resolve("public/fredrun");
const OUTPUT_FILE = "luki-colombia-run.webp";
...
function alphaBounds(data, channels, imageWidth, rectangle) {
  ...
  if (right < left || bottom < top) {
    throw new Error(`Leerer Luki-Frame bei ${rectangle.left},${rectangle.top}.`);
  }
...
async function main() {
  ...
  if (sha256(source) !== EXPECTED_SOURCE_HASH) {
    throw new Error(`Unerwartetes Luki-Spritesheet: ${path.basename(SOURCE_PATH)}`);
  }
  ...
  await writeFile(
    path.join(OUTPUT_DIRECTORY, "luki-manifest.json"),
