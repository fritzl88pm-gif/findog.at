...
const SOURCE_PATH = process.argv[2] ?? "C:/Users/conta/Downloads/jqa-dance_gangnam.png";
const EXPECTED_SOURCE_HASH = "8D2E3FB26AD4E7E28DF590C92E8679F60CF9EA8CA7B38D7931863163973F502F";
const OUTPUT_DIRECTORY = path.resolve("public/fredrun");
const OUTPUT_FILE = "jqa-dance-gangnam.webp";
...
function alphaBounds(data, channels, imageWidth, rectangle) {
  ...
  if (right < left || bottom < top) {
    throw new Error(`Leerer JQA-Frame bei ${rectangle.left},${rectangle.top}.`);
  }
...
async function main() {
  ...
  if (sha256(source) !== EXPECTED_SOURCE_HASH) {
    throw new Error(`Unerwartetes JQA-Spritesheet: ${path.basename(SOURCE_PATH)}`);
  }
  ...
  await writeFile(
    path.join(OUTPUT_DIRECTORY, "jqa-dance-gangnam-manifest.json"),
