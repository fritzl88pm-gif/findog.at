...
Fred history is stored separately from the internal agent history, with explicit WeKnora provenance. Configure the WeKnora channel to POST `message_sent` and `message_received` events to `https://findog.at/api/webhooks/weknora` and use the same `WEKNORA_FRED_WEBHOOK_SECRET` for its `X-WeKnora-Signature` HMAC. The authenticated iframe bridge binds a WeKnora session to exactly one Findog user; webhook events received before that binding remain in a short-lived inbox and are attached after the first authenticated bridge event. The sidebar supports opening, selecting, and deleting Fred histories. A stored Fred transcript is read-only because the current upstream embed does not expose a host command for reopening an older session inside the iframe.
