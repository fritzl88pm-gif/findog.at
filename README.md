...
| `OPENAI_LIVE_VOICE` | Optional | Voice for Fred Live, fixed when a session starts. Defaults to the male `stone`; `beacon`, `bossa`, `cinder`, `delta`, `gleam`, `marin`, `meridian`, `quartz`, `ripple`, `tempo`, `vesper` and `willow` are also accepted, and an unknown value falls back to the default. |

Fred Live is billed by OpenAI per second of voice duration.

Fred Live runs GPT-Live with client delegation: GPT-Live carries the spoken conversation, and every factual question is answered by the QuickFred knowledge base. `session.delegation.created` names no task, so the browser reconstructs the question from the transcript events, posts it to the admin-only `/api/fred-live/ask` route, and hands the answer back as `session.commentary.append` events. GPT-Live rejects a context append above 500 tokens and stays silent about it, so answers are split into sentence-aligned appends that share the delegation id. Fred Live therefore needs the four `WEKNORA_QUICKFRED_*` values in addition to the OpenAI key; without them the voice session connects but every delegated question fails. Fred opens the call himself: the browser asks for a greeting once `session.started` arrives, and while the knowledge base runs it sends spaced short filler lines so the line does not fall silent for seconds. A context append guides the model rather than dictating to it, so both the greeting and the fillers are requests, not guarantees.
...
FredRun access restrictions are keyed by Supabase Auth user ID and checked before progress or highscore access. Block records remain server-only, database triggers reject writes for blocked users, and moderation events retain the actor, reason, provenance, and affected-row count without hard-coding account emails in the application.
...
Generation slots refill independently as each job finishes. A separate single-slot control lane polls only `/stop` commands, so cancellation remains available even when all generation slots are occupied. Apply migration `20260904220159_telegram_worker_control_lane.sql` before deploying this worker; the existing claim RPC remains compatible with older workers. Per-chat serialization, attempt limits and lease checks still apply. Stop detection normally takes up to one queue poll (2 seconds) plus one running-job heartbeat (10 seconds).

Each Fred generation has a 12-minute total deadline and a 5-minute deadline without new answer text or changed research/execution steps. Network keepalives and repeated status messages do not extend these deadlines. Timeouts abort the upstream and use the existing retry/reconciliation path; they are not user cancellations, and an already persisted answer remains deliverable. If the generator still fails to settle within 30 seconds after abort, the worker becomes unhealthy and shuts down (forced exit after another 30 seconds), rather than releasing its slot while orphan work could still write. Attachment preprocessing and delivery retain their existing independent bounds.
...
Both health endpoints return 503 until both processing lanes have initialized, after a claim failure, when a lane stops reporting for 90 seconds, or during shutdown. Ordinary long-running jobs remain healthy while their scheduler and generation deadlines are functioning.

Verified rollout on 5 September 2026 (Europe/Vienna): migration `20260904220159`, worker revision `de8c023`, rolling update completed at 00:04:28. Both lanes initialized successfully and the old container stopped. Validation: 457 relevant application tests, both typechecks and builds, plus disposable PostgreSQL 17 tests for control claims, busy-chat isolation, stale leases, recovery and simultaneous claims without duplicate ownership. `supabase/tests/telegram_worker_control_lane.sql` contains the repeatable database regression checks.
