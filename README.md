...
Scanning is a one-shot batch evaluation and does not create a chat history. An authenticated user can submit up to five JPEG, PNG, WebP or GIF images (5 MiB each) and five PDFs (10 MiB each). The server validates counts, MIME types, file signatures and request size, detects byte-identical duplicates by SHA-256, and sends the complete unique batch directly as Base64 data in one request to `google/gemini-3.5-flash` through OpenRouter. It does not use OpenRouter's persistent file upload or a restrictive structured-output schema. Gemini inspects every PDF page, including rotated pages, recognizes independent documents in a combined PDF and returns the finished German Markdown report directly. Tables and categories are used where useful, but the report records the amounts actually shown on a document instead of requiring fixed net, VAT and gross fields. A missing explicit net total is therefore not treated as an extraction failure. Comparable, unambiguous totals are grouped by currency without currency conversion. Foreign-language descriptions are translated into German while names, identifiers and financial values remain unchanged. Files and reports remain in the current browser workflow only and are never written to Supabase. The resulting report can be exported through the existing authenticated PDF tool.
